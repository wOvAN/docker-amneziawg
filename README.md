...
**This is a fork of [linuxserver/docker-wireguard](https://github.com/linuxserver/docker-wireguard) with the VPN engine replaced by [AmneziaWG](https://github.com/amnezia-vpn/amneziawg-go) (AWG 3.1)** — the management approach (env vars, templates, peer/QR flow, s6 services) is unchanged, only the tooling differs (`awg`/`awg-quick`/`amneziawg-go` instead of `wg`/`wg-quick`) and generated confs carry AWG 3.1 obfuscation parameters.
...
If you want to use the amneziawg kernel module instead of the userspace fallback, you need to compile and load it on your host. The kernel module is maintained separately: [amnezia-vpn/amneziawg-linux-kernel-module](https://github.com/amnezia-vpn/amneziawg-linux-kernel-module). Follow the build instructions in that repository for your kernel version, then load it with `modprobe amneziawg`. The module must be at least v3.1, since the generated configs use the AWG 3.1 parameters (`RandomTrailers`, `DisableCookies`) which older modules do not understand. When the module is active, the container can drop the `SYS_MODULE` capability (but you may still need it if your host does not auto-load the iptables modules).

This image generates server and peer configs with AmneziaWG 3.1 obfuscation parameters (junk packets `Jc`/`Jmin`/`Jmax`, message padding `S1`-`S4`, magic headers `H1`-`H4`, `HeaderProtectionKey`, content padding, randomized rekey/keepalive timings, an `I1` CPS protocol-imitation signature that prefixes handshake packets with fake DNS traffic so they blend in with regular internet traffic, `RandomTrailers` which appends a random number of bytes to the end of each packet, and `DisableCookies` which suppresses handshake cookie replies) which are randomly generated once per server and shared between the server and peer configs, just like the server keys. The generated peer configs can be imported into the Amnezia VPN clients (desktop, mobile, iOS).
...
The AmneziaWG 3.1 obfuscation parameters are randomly generated once per server (along with the server keys) and stored in `/config/server/awgparams`. Both templates reference this file with a `$(cat /config/server/awgparams)` line, so the same parameters end up in the server conf and all peer confs (S1-S4, H1-H4, I1, HeaderProtectionKey, RandomTrailers and DisableCookies must match between server and clients). Advanced users can replace that line in a template with explicit values, or edit the awgparams file and delete wg0.conf to regenerate the confs.
...
docker run -d \
  ...
  -e ALLOWEDIPS=0.0.0.0/0,::/0 `#optional` \
...
| `-e ALLOWEDIPS=0.0.0.0/0,::/0` | The IPs/Ranges that the peers will be able to reach using the VPN connection. Default is `0.0.0.0/0,::/0` which routes all traffic (including IPv6) through the VPN — required to prevent IPv6 DNS/WebRTC leaks. For split tunneling, set this to only the IPs you would like to use the tunnel AND the ip of the server's awg ip, such as `10.13.13.1,::/0`. |
...
* **17.08.26:** - **Potentially Breaking Change:** Upgrade to AmneziaWG 3.1. Generated configs now include the new `RandomTrailers` and `DisableCookies` parameters (both enabled, matching the official Amnezia client defaults). Existing installs are migrated automatically on first start, but **existing peers must re-import their configs** to connect.
